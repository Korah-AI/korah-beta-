...
enum StudyDataManager {
    
    /// Generic save function for Codable arrays
    static func save<T: Codable>(_ items: [T], forKey key: String) -> Bool {
    ...
    /// Generic load function for Codable arrays
    ...
    static func append<T: Codable>(_ item: T, toKey key: String) -> Bool {
        var items: [T] = load(forKey: key) ?? []
        items.append(item)
        return save(items, forKey: key)
    }
    
    /// Remove item from array by ID and save
    static func remove<T: Codable & Identifiable>(withId id: T.ID, fromKey key: String, type: T.Type) -> Bool where T.ID: Equatable {
        var items: [T] = load(forKey: key) ?? []
